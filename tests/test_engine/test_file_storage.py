#!/usr/bin/python3
"""Unittest pour tester FileStorage"""
...
class storageTest(unittest.TestCase):
    ...
    def test_save(self):
        """tester la sauvegarde du nouvel obj"""
        model = BaseModel()
        x = model.id
    ...
    def test_newobj(self):
        """tester soi-même.__ojects dans le stockage de fichiers
"""
        storage = FileStorage()
        model = BaseModel()
        x = model.id
    ...
    def help_test_all(self, classname):
        """Helper teste la méthode all() pour le nom de classe."""
        storage = FileStorage()
        o = eval(classname)()
        storage.new(o)
